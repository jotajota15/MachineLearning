...
class Agent():
    ...
    def __update_q_table(self,current_state,reward,new_state,action):
        acc_val = self.STATS[QT][new_state].max()
        acc_val = (reward +  self.STATS[DF] * (acc_val)   - self.STATS[QT][current_state][action]) * self.STATS[LR] 
        self.STATS[QT][current_state][action] += acc_val
    ...
    def step(self, env, learn=True):
        randomAction = False
        action = 0
        current_state = env.get_state() # To update q-learning table
        ...
        # If that state hasn´t been visited yet, has to be registered
        if new_state not in self.STATS[QT]:
            self.new_key(new_state)
        # Update the q table if learn = true
        if learn:
            reward = env_tuple[0]
